...
def attention_decoder(decoder_inputs, initial_state, attention_states, cell,
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      # Run the RNN.
      cell_output, state = cell(x, state)#解码输入
      ...
      # MODIFIED ADD END
        ...
        attention_weights_history.append(attn_weights)
        # MODIFIED ADD END
      #aaaa=tf.constant([1.0,2.0,3.0,4.0])
	 
      #with tf.Session() as sess:
      #aaaa = tf.Variable(tf.zeros([3]), name="aaaa")
      #print(aaaa.eval())#sess.run(aaaa))
	  
      with variable_scope.variable_scope("AttnOutputProjection"): #修改“AttnOutputProjection”这个参数名，就可以使用预训练
        output = linear([cell_output] + attns, output_size, True)
        #print(output.eval())
       
        #sess1.close()
        #print ("output type:{}".format(type(output)))
        #print ("output:{}".format(sess1.run(output)))
      if loop_function is not None:
        prev = output
      outputs.append(output)
