...
class Seq2SeqModel(object):
    ...
    def __init__(self, encoder_masks, encoder_inputs_tensor, 
        ...
        def seq2seq_f(lstm_inputs, decoder_inputs, seq_length, do_decode):
            ...
            '''if use_gru: #5.27 之前编码模块是lstm,修改成可用gru的
                print("using GRU CELL in encoder")
                lstm_fw_cell = tf.contrib.rnn.core_rnn_cell.GRUCell(num_hidden)
                lstm_bw_cell = tf.contrib.rnn.core_rnn_cell.GRUCell(num_hidden)'''            
            ...
            #print ("\n")
            #print ("lstm_inputs: "+ str(len(lstm_inputs))+ " " + str(lstm_inputs[0].get_shape()))
            #print ("lstm_inputs:{}".format())
            bb = np.array(decoder_inputs)
            #print ("decoder_inputs:" + str(len(decoder_inputs))+ " " + str(decoder_inputs[0].get_shape()))
            #print ("output_state_fw:{}".format(output_state_fw.get_shape()))
            #print ("pre_encoder_inputs:"+ str(len(pre_encoder_inputs)) + " " + str(pre_encoder_inputs[0].get_shape()))
            
            encoder_inputs = [e*f for e,f in zip(pre_encoder_inputs,encoder_masks[:seq_length])]
            top_states = [array_ops.reshape(e, [-1, 1, num_hidden*2]) for e in encoder_inputs]
               
            attention_states = array_ops.concat(top_states, 1)
            initial_state = tf.concat(axis=1, values=[output_state_fw, output_state_bw])
            #print ("attention_states:{}".format(attention_states.get_shape()))
            #print ("initial_state:{}".format(initial_state.get_shape()))
