...
fw_correct = codecs.open(os.getcwd() + "/result/correct.txt", 'w', "utf-8")
fw_imcorrect = codecs.open(os.getcwd() + "/result/imcorrect.txt", 'w', "utf-8")
#fw_data = codecs.open(output_dir + "/data.txt", 'w', "utf-8")
    ...
    def __init__(self,
        ...
        if not self.forward_only:
            ...
            with tf.device(gpu_device_id):
                ...
                opt = tf.train.AdadeltaOptimizer(learning_rate=initial_learning_rate) #learning_rate=0.001, rho=0.95, epsilon=1e-08
                for b in xrange(len(buckets)):
                    if self.reg_val > 0:
                        reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)#从一个结合中取出全部变量，是一个列表;损失函数上加上正则项是防止过拟合的
                        logging.info('Adding %s regularization losses', len(reg_losses))
                        logging.debug('REGULARIZATION_LOSSES: %s', reg_losses)
                        loss_op = self.reg_val * tf.reduce_sum(reg_losses) + self.attention_decoder_model.losses[b] #tf.reduce_sum 沿着指定轴求和
                    else:
                        loss_op = self.attention_decoder_model.losses[b]

                    gradients, params = zip(*opt.compute_gradients(loss_op, params)) 
                    if self.clip_gradients:
                        gradients, _ = tf.clip_by_global_norm(gradients, max_gradient_norm) #函数控制梯度的大小,避免梯度膨胀的问题
    ...
    def chinese_display(self, filename, output, fw, flag):
        ...
        for c in output:
            if c > 2 and c < self.target_vocab_size+1 :#4450:#24:#951: 
                ...
                lst.append( c.encode("utf-8") )#c.decode("raw_unicode_escape").encode("utf-8")
                fw.write( c )
                #print ("11111111111")
                #print (lst)
        s = "".join(lst) 
        ...
        if self.phase == 'test':
            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                for idx, output, ground in zip(range(len(grounds)), step_outputs, grounds):
                    ...
                    if num_incorrect < 1:
                        self.chinese_display(file_list, ground_valid, fw_correct, 1)
                        self.chinese_display(file_list, output_valid, fw_correct, 0)
                    else:
                        self.chinese_display(file_list, ground_valid, fw_imcorrect, 1)
                        self.chinese_display(file_list, output_valid, fw_imcorrect, 0)

                    
                    if num_incorrect < 1:
                        num_correct = num_correct + 1
                logging.info('%f out of %d correct, precision=%f, %f, %f' %(num_correct, num_total, num_correct/(num_total+1), num_correct1, num_correct1/(num_total+1)))
                ...
                for epoch in range(self.num_epoch):
                   ...
                   for i,batch in enumerate(self.s_gen.gen(self.batch_size)):
                        ...
                        if current_step % self.steps_per_checkpoint == 0:
                            ...
                            logging.info("global step %d step-time %.2f loss %f  perplexity %.2f precision_ave %.4f" %  (self.global_step.eval(), step_time, loss, perplexity, precision_ave/self.steps_per_checkpoint))
                            precision_ave = 0.0
    ...
    def step(self, encoder_masks, img_data, zero_paddings, decoder_inputs, target_weights,
        ...
        if not forward_only: #train
            output_feed  = [self.updates[bucket_id],  # Update Op that does SGD.
                    ...
                    self.attention_decoder_model.losses[bucket_id],self.summaries_by_bucket[bucket_id]]
