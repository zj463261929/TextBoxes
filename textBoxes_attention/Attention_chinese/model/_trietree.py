...
class TrieNode:
    ...
    def __init__(self):
        ...
        global NodeCount
        NodeCount += 1
        self.model = None #kenlm.Model('dict.lm')
    ...
    def load_userdict(self, f):
        ...
        if isinstance(f, string_types):
            ...
            '''with open(cache_file_dict, 'rb') as cf:
    ...
    def initialize(self, DICTIONARY):
        ...
        with self.lock:
            ...
            t1 = time.time()
            if self.cache_file_dict:
                cache_file_dict = self.cache_file_dict
            # default dictionary
            cache_file_dict = "dict.cache"
            ...
            # custom dictionary
                ...
                cache_file_dict = "dict.u%s.cache" % md5(abs_path.encode('utf-8', 'replace')).hexdigest() #str.encode(encoding='UTF-8',errors='strict')
            '''
            
            #cache_file_dict = os.path.join(self.tmp_dir or tempfile.gettempdir(), cache_file_dict) # gettempdir()则用于返回保存临时文件的文件夹路径。
            #print ("tempfile.gettempdir():{}\n".format(tempfile.gettempdir()))  #/tmp  
            # prevent absolute path in self.cache_file_dict
            tmpdir = os.path.dirname(cache_file_dict) #返回最后的文件名  /tmp
            #self.cache_file_dict = cache_file_dict
            #print tmpdir

            load_from_cache_fail = True #第二次走着
            if os.path.isfile(cache_file_dict) and (abs_path == DEFAULT_DICT or
                os.path.getmtime(cache_file_dict) > os.path.getmtime(abs_path)):
                default_logger.debug("Loading model from cache %s" % cache_file_dict) # /tmp/jieba.cache
                    
                try: 
                    #print (cache_file_dict) #/tmp/jieba.cache
                    with open(cache_file_dict, 'rb') as cf:
                        self.words, self.freqs = marshal.load(cf)  #二进制流反序列化为对象
            ...
            if load_from_cache_fail: #第一次走这
                ...
                with wlock:
                    ...
                    default_logger.debug("Dumping model to file cache %s" % cache_file_dict)
                    ...
                    try:
                        ...
                        _replace_file(fpath, cache_file_dict)
                    except Exception:
    ...
    def check_initialized(self, DICTIONARY, lm):
...
def construction_trietree(DICTIONARY, lm): #(words, freqs):
    
    WordCount = 0 
    # read dictionary file into a trie
    trie = TrieNode()
    
    cache_path_dict = os.path.join(os.getcwd(), "dict.cache")
     
    #print  cache_path
    if os.path.exists(cache_path_dict):
        trie.cache_file_dict = "dict.cache"
        ...
        if os.path.exists(DICTIONARY):      
            ...
            print (" The dict path cannot be found: %s." % DICTIONARY)
            return None

    cache_path_lm = os.path.join(os.getcwd(), lm)       
    if os.path.exists(cache_path_lm):
        trie.model = kenlm.Model(cache_path_lm) #"dict.lm"
      
    else:
        print (" The lm path cannot be found: %s." % cache_path_lm)
        return None

    trie.check_initialized(DICTIONARY, lm)
    ...
    for i in range(min(len(trie.words),len(trie.freqs))):
        ...
        WordCount += 1
        trie.insert( uniform(trie.words[i]), uniform(trie.freqs[i])) #10
        #print word, word_fre   
    #ann_file.close()
...
#step2.3：               
def postProcessing(TARGET, words, distance, freq, trie):
    '''words = []
    distance = []
        ...
        if len(words_min)==1: #表示只有一个最接近目标word
            ...
            for i in range(len(words_min)):
                ...
                if len(TARGET)==len(words_min[i]):
                    ...
                    for c in words_min:
                        #print type(c)
                        dis = trie.model.score(c, bos=False, eos=False)
                        #print dis
                        if dis>dis_max:
                            dis_max = dis
                            word_max = c
...
def correct_word(TARGET, MAX_COST, trie): #MAX_COST 一般取3
    ...
    word = postProcessing(TARGET, words, dis, freq, trie) #后处理
    #print ("word:{}\n".format(word))
